...
public class OthelloState implements State {
	...
	private float CandStarDifferential() {
		float diff = 0;
	...
	private float cornerAroundDifferential() {
		float diff = 0;
		diff += countCornerAround(0, 0, 1, 1);
		diff += countCornerAround(0, dimension-1, -1, 1);
		diff += countCornerAround(dimension-1, 0, 1, -1);
		diff += countCornerAround(dimension-1, dimension-1, -1, -1);
